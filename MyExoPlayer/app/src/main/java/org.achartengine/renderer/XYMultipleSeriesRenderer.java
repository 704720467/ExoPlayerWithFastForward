...
public class XYMultipleSeriesRenderer extends DefaultRenderer {
    ...
    public void initAxesRangeForScale(int i) {
        ...
        this.initialRange.put(Integer.valueOf(i), range);
        this.mYTitle[i] = "";
    ...
    public void setXAxisMin(double min, int scale) {
        if(!this.isMinXSet(scale)) {
            ((double[])this.initialRange.get(Integer.valueOf(scale)))[0] = min;
        }
    ...
    public void setXAxisMax(double max, int scale) {
        if(!this.isMaxXSet(scale)) {
            ((double[])this.initialRange.get(Integer.valueOf(scale)))[1] = max;
        }
    ...
    public void setYAxisMin(double min, int scale) {
        if(!this.isMinYSet(scale)) {
            ((double[])this.initialRange.get(Integer.valueOf(scale)))[2] = min;
        }
    ...
    public void setYAxisMax(double max, int scale) {
        if(!this.isMaxYSet(scale)) {
            ((double[])this.initialRange.get(Integer.valueOf(scale)))[3] = max;
        }
    ...
    public synchronized void addYTextLabel(double y, String text, int scale) {
        ((Map)this.mYTextLabels.get(Integer.valueOf(scale))).put(Double.valueOf(y), text);
    }

    public synchronized void removeYTextLabel(double y, int scale) {
        ((Map)this.mYTextLabels.get(Integer.valueOf(scale))).remove(Double.valueOf(y));
    }
    ...
    public synchronized String getYTextLabel(Double y, int scale) {
        return (String)((Map)this.mYTextLabels.get(Integer.valueOf(scale))).get(y);
    }
    ...
    public synchronized Double[] getYTextLabelLocations(int scale) {
        return (Double[])((Map)this.mYTextLabels.get(Integer.valueOf(scale))).keySet().toArray(new Double[0]);
    }
    ...
    public synchronized void clearYTextLabels(int scale) {
        ((Map)this.mYTextLabels.get(Integer.valueOf(scale))).clear();
    }
    ...
    public double[] getInitialRange(int scale) {
        return (double[])this.initialRange.get(Integer.valueOf(scale));
    }
