...
public class DoughnutChart extends RoundChart {
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        for(int category = 0; category < cLength; ++category) {
            ...
            for(int currentAngle = 0; currentAngle < sLength; ++currentAngle) {
                total += this.mDataset.getValues(category)[currentAngle];
                titles[currentAngle] = this.mDataset.getTitles(category)[currentAngle];
            }

            float var33 = this.mRenderer.getStartAngle();
            ...
            for(int i = 0; i < sLength; ++i) {
                ...
                canvas.drawArc(oval, var33, angle, true, paint);
                this.drawLabel(canvas, this.mDataset.getTitles(category)[i], this.mRenderer, prevLabelsBounds, this.mCenterX, this.mCenterY, shortRadius, longRadius, var33, angle, left, right, this.mRenderer.getLabelsColor(), paint, true, false);
                var33 += angle;
            }
