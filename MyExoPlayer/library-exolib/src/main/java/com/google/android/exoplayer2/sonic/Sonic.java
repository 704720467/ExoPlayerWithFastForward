...
public class Sonic {
    ...
    public void allocateStreamBuffers(int sampleRate, int numChannels) {
        isDo = false;
    ...
    private void adjustPitch(int originalNumOutputSamples) {
        ...
        while (numPitchSamples - position >= maxRequired) {
            ...
            if (pitch >= 1.0f) {
                overlapAdd(newPeriod, numChannels, outputBuffer, numOutputSamples, pitchBuffer, position, pitchBuffer, position + period - newPeriod);
            } else {
                separation = newPeriod - period;
                overlapAddWithSeparation(period, numChannels, separation, outputBuffer, numOutputSamples, pitchBuffer, position, pitchBuffer, position);
            }
    ...
    private void adjustRate(float rate, int originalNumOutputSamples) {
        ...
        for (position = 0; position < numPitchSamples - 1; position++) {
            while ((oldRatePosition + 1) * newSampleRate > newRatePosition * oldSampleRate) {
                ...
                for (int i = 0; i < numChannels; i++) {
                    outputBuffer[numOutputSamples * numChannels + i] = interpolate(pitchBuffer, position + i, oldSampleRate, newSampleRate);
                }
                newRatePosition++;
                numOutputSamples++;
            }
